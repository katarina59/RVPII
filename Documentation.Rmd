...
```{r Filtering Cases Where the Police Arrival Time Precedes the Incident Time}
df_clean <- df_clean %>%
...
```{r Displaying Statistical Information on Incident Times}
df_clean %>% summarise(
...
```{r Visualization of the Distribution by Police Precincts}
...
```{r Visualization of the Distribution by Case Resolution Type}
...
```{r Visualization of the Distribution by Priority}
...
```{r Visualization of the Distribution by 911 Call Type}
...
```{r Histograms of Longitude Distribution for Cleaned Values}
...
```{r Histograms of Latitude Distribution for Cleaned Values}
# Sample 50% of cleaned location data for histogram
location_clean_sample <- df_clean_ll %>%
  select(Blurred_Longitude, Blurred_Latitude) %>%
  sdf_sample(fraction = 0.5, replacement = FALSE) %>%
...
```{r Grouping of initial call categories into supercategories}
...
```{r Grouping of final call categories into supercategories}
df_clean_ll <- df_clean_ll %>%
...
```{r Distribution of cases where the final categorization matched the initial one}
...
```{r Calculating Response Time to an Incident}
...
```{r Histogram Visualization to Examine the Distribution of Response Times}
...
```{r Creating the Response Speed Label}
...
```{r Distribution of Response Speed}
...
```{r Visualization of the Relationship Between Predictor Features and the Target Feature}
...
``` {r Display of Logistic Regression Model Performance for Different Scenarios}
cv_metrics %>% 
...
``` {r Display of Random Forest Model Performance for Different Scenarios}
rf_cv_metrics %>% 
...
``` {r Display of Decision Tree Model Performance for Different Scenarios}
dt_cv_metrics %>% 
...
```{r Evaluating the Classification Performance of Different Methods}
...
```{r Preparing the Data for Clustering}
...
```{r Determining the Optimal Cluster Size Using the Elbow Method}
